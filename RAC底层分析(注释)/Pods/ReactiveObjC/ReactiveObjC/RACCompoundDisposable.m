...
- (void)addDisposable:(RACDisposable *)disposable {
    ...
    //  单个销毁者 如果为空， 或者 已经被处理（disposable.disposed == yes）就直接 return
	...
	{
        // 以下一堆判断就是为数组里面添加值
        //  _disposed 会在 【disposable dispose】的方法里 进行赋值处理
		if (_disposed) {
			shouldDispose = YES;
		} else {
            // 上限个数为2
