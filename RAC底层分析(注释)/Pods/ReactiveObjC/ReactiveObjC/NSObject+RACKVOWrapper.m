...
- (RACDisposable *)rac_observeKeyPath:(NSString *)keyPath options:(NSKeyValueObservingOptions)options observer:(__weak NSObject *)weakObserver block:(void (^)(id, NSDictionary *, BOOL, BOOL))block {
    ...
    // 获取keyPath 的路由,比如vc有一个 Person类， RACObserve(vc,person.name)
   ...
   // 获取 被监听对象的 ”属性列表“
    ...
    /** --------------------------------------------------------------- */
    // KVO: 面向对象 ， 谁调用谁被监听
    // 此处把所有的相关参数传入，如：观察者，被观察对象，keyPath 等，用 RACKVOTrampoline 进行了封装，实质的监听工作就是在此类里面进行
...
//         RACKVOTrampoline 里的响应监听方法 - (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context
        // 会把新值 Block 到这里来
        ...
        // block  返回到 （NSObject + RACPropertySubscribing) 的信号量定义 block里， 并 sendNext
